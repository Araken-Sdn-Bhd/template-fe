var __defProp = Object.defineProperty;
var __getOwnPropSymbols = Object.getOwnPropertySymbols;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __propIsEnum = Object.prototype.propertyIsEnumerable;
var __defNormalProp = (obj, key, value) => key in obj ? __defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var __spreadValues = (a, b) => {
  for (var prop in b || (b = {}))
    if (__hasOwnProp.call(b, prop))
      __defNormalProp(a, prop, b[prop]);
  if (__getOwnPropSymbols)
    for (var prop of __getOwnPropSymbols(b)) {
      if (__propIsEnum.call(b, prop))
        __defNormalProp(a, prop, b[prop]);
    }
  return a;
};
import { u as useLocalConfig } from "../../components/va-config/VaConfig.mjs";
import { a as useGlobalConfig } from "../global-config/global-config.mjs";
import { computed } from "vue";
const useComponentConfigProps = (component, originalProps) => {
  const localConfig = useLocalConfig();
  const { globalConfig } = useGlobalConfig();
  const instancePreset = computed(() => originalProps.preset);
  const getPresetProps = (presetName) => {
    var _a, _b, _c;
    return (_c = (_b = (_a = globalConfig.value.components) == null ? void 0 : _a.presets) == null ? void 0 : _b[component.name]) == null ? void 0 : _c[presetName];
  };
  return computed(() => {
    var _a, _b;
    const globalConfigProps = __spreadValues(__spreadValues({}, (_a = globalConfig.value.components) == null ? void 0 : _a.all), (_b = globalConfig.value.components) == null ? void 0 : _b[component.name]);
    const localConfigProps = localConfig.value.reduce((finalConfig, config) => config[component.name] ? __spreadValues(__spreadValues({}, finalConfig), config[component.name]) : finalConfig, {});
    const presetName = instancePreset.value || localConfigProps.preset || globalConfigProps.preset;
    const presetProps = presetName && getPresetProps(presetName);
    return __spreadValues(__spreadValues(__spreadValues({}, globalConfigProps), localConfigProps), presetProps);
  });
};
export { useComponentConfigProps as u };
//# sourceMappingURL=component-config.mjs.map
