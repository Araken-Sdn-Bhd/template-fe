export { VaTab } from './components/VaTab';
export declare const VaTabs: import("vue").DefineComponent<{
    modelValue: {
        type: (StringConstructor | NumberConstructor)[];
        default: null;
    };
    left: {
        type: BooleanConstructor;
        default: boolean;
    };
    right: {
        type: BooleanConstructor;
        default: boolean;
    };
    center: {
        type: BooleanConstructor;
        default: boolean;
    };
    grow: {
        type: BooleanConstructor;
        default: boolean;
    };
    disabled: {
        type: BooleanConstructor;
        default: boolean;
    };
    hideSlider: {
        type: BooleanConstructor;
        default: boolean;
    };
    vertical: {
        type: BooleanConstructor;
        default: boolean;
    };
    color: {
        type: StringConstructor;
        default: string;
    };
    prevIcon: {
        type: StringConstructor;
        default: string;
    };
    nextIcon: {
        type: StringConstructor;
        default: string;
    };
    preset: {
        type: StringConstructor;
        default: undefined;
    };
    stateful: {
        type: import("vue").PropType<boolean>;
        default: boolean;
    };
}, {
    wrapper: import("vue").ShallowRef<HTMLElement | undefined>;
    container: import("vue").ShallowRef<HTMLElement | undefined>;
    tabs: import("vue").ShallowRef<HTMLElement | undefined>;
    tabsList: import("vue").Ref<import("./types").TabComponent[]>;
    sliderHeight: import("vue").Ref<number | null>;
    sliderWidth: import("vue").Ref<number | null>;
    sliderOffsetX: import("vue").Ref<number>;
    sliderOffsetY: import("vue").Ref<number>;
    showPagination: import("vue").Ref<boolean>;
    tabsContentOffset: import("vue").Ref<number>;
    startingXPoint: import("vue").Ref<number>;
    animationIncluded: import("vue").Ref<boolean>;
    colorComputed: import("vue").ComputedRef<string>;
    tabConfig: {
        VaTab: {
            color: string;
        };
    };
    computedClass: import("vue").ComputedRef<{
        'va-tabs__container--left': boolean;
        'va-tabs__container--right': boolean;
        'va-tabs__container--center': boolean;
        'va-tabs__container--grow': boolean;
        'va-tabs__container--disabled': boolean;
    }>;
    computedTabsClass: import("vue").ComputedRef<{
        'va-tabs--vertical': boolean;
    }>;
    tabSelected: import("vue").WritableComputedRef<import("./types").TabSelected>;
    sliderStyles: import("vue").ComputedRef<import("vue").StyleValue>;
    paginationControlledStyles: import("vue").ComputedRef<import("vue").StyleValue>;
    disablePaginationLeft: import("vue").ComputedRef<boolean>;
    disablePaginationRight: import("vue").ComputedRef<boolean>;
    resetSliderSizes: () => void;
    moveToTab: (tab: import("./types").TabComponent) => void;
    updateStartingXPoint: () => void;
    updateTabsState: () => void;
    updatePagination: () => void;
    movePaginationLeft: () => void;
    movePaginationRight: () => void;
    updateSlider: (tab: import("./types").TabComponent) => void;
    includeAnimation: () => void;
    redrawTabs: () => void;
    selectTab: (tab: import("./types").TabComponent) => void;
    tp: <Key extends string | undefined>(key: Key) => Key;
    t(key: string, values?: Record<string, string | number | boolean | undefined> | undefined): string;
}, unknown, {}, {}, import("vue").ComponentOptionsMixin, import("vue").ComponentOptionsMixin, ("update:modelValue" | "click:next" | "click:prev")[], "update:modelValue" | "click:next" | "click:prev", import("vue").VNodeProps & import("vue").AllowedComponentProps & import("vue").ComponentCustomProps, Readonly<import("vue").ExtractPropTypes<{
    modelValue: {
        type: (StringConstructor | NumberConstructor)[];
        default: null;
    };
    left: {
        type: BooleanConstructor;
        default: boolean;
    };
    right: {
        type: BooleanConstructor;
        default: boolean;
    };
    center: {
        type: BooleanConstructor;
        default: boolean;
    };
    grow: {
        type: BooleanConstructor;
        default: boolean;
    };
    disabled: {
        type: BooleanConstructor;
        default: boolean;
    };
    hideSlider: {
        type: BooleanConstructor;
        default: boolean;
    };
    vertical: {
        type: BooleanConstructor;
        default: boolean;
    };
    color: {
        type: StringConstructor;
        default: string;
    };
    prevIcon: {
        type: StringConstructor;
        default: string;
    };
    nextIcon: {
        type: StringConstructor;
        default: string;
    };
    preset: {
        type: StringConstructor;
        default: undefined;
    };
    stateful: {
        type: import("vue").PropType<boolean>;
        default: boolean;
    };
}>> & {
    "onUpdate:modelValue"?: ((...args: any[]) => any) | undefined;
    "onClick:next"?: ((...args: any[]) => any) | undefined;
    "onClick:prev"?: ((...args: any[]) => any) | undefined;
}, {
    preset: string;
    modelValue: string | number;
    stateful: boolean;
    vertical: boolean;
    color: string;
    right: boolean;
    left: boolean;
    center: boolean;
    disabled: boolean;
    grow: boolean;
    hideSlider: boolean;
    prevIcon: string;
    nextIcon: string;
}>;
export * from './types';
